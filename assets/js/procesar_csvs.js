async function cargarRegalos() {
    ...
    if (savedRegalos) {
        try {
            const regalos = JSON.parse(savedRegalos);
            renderRegalos(regalos);
            return;
        } catch (e) {
            console.error('Error loading regalos from localStorage:', e);
        }
    }
    ...
    Papa.parse(csvData, {
        ...
        complete: function (results) {
            ...
            localStorage.setItem('wedding-regalos', JSON.stringify(data));
        },
...
function renderHoteles(data) {
    $('#hoteles').empty();
    $('#modalesHoteles').empty();
    ...
    if ($('#hoteles').children().length > 0) {
        ...
        if (typeof Swiper !== 'undefined' && $('.js-swiper-card-grid').length > 0) {
            // Destruir swiper existente si hay uno
            if ($('.js-swiper-card-grid')[0].swiper) {
                $('.js-swiper-card-grid')[0].swiper.destroy(true, true);
            }
            // Crear nuevo swiper
    ...
    localStorage.setItem('wedding-hoteles', JSON.stringify(data));
...
async function cargarHoteles() {
    ...
    if (savedHoteles) {
        try {
            const hoteles = JSON.parse(savedHoteles);
            renderHoteles(hoteles);
            return;
        } catch (e) {
            console.error('Error loading hoteles from localStorage:', e);
        }
    }
    ...
    Papa.parse(csvData, {
        ...
        complete: function (results) {
            ...
            // Llamar a renderHoteles para renderizar
            renderHoteles(data);
        },
        error: function (error) {
            alert("Error parsing CSV file: " + error.message);
